            ...
            <div class="card" id="List">
                    ...
                    <div class="row g-4 align-items-center">
                        ...
                        <div class="col-sm-auto">
                            <div class="d-flex flex-wrap align-items-start gap-2">
                                <a href="{{ route('lecture.assignment.questions', $question->assignment_id) }}"
                ...
                <div class="card-body">
                    <form method="POST" class="row g-3"
                        action="{{ route('lecture.assignment.update_question', $question->id) }}">
                        @csrf
                        @method('PUT')
                        <div class="col-md-6">
                            ...
                            <select name="type" id="question-type" class="form-select" required>
                                ...
                                <option value="open" {{ $question->type == 'open' ? 'selected' : '' }}>Open Question
                                </option>
                                <option value="file" {{ $question->type == 'file' ? 'selected' : '' }}>File Upload
                                </option>
                            </select>
                            @error('type')
                        ...
                        <div id="choices-container" class="col-md-12 mb-2"
                                ...
                                <table id="item_table" style="width: 100%">
                                    @php
                                        $oldChoices =
                                            old('choices') ??
                                            ($question->choices
                                                ? collect($question->choices)->pluck('title', 'id')->toArray()
                                                : [1 => '']);
                                        $oldAnswers =
                                            old('answers') ??
                                            ($question->choices
                                                ? collect($question->choices)
