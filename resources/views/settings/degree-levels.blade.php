            ...
            <div class="card" id="List">
                <div class="card-header border-bottom-dashed">
                    ...
                    <div class="row g-4 align-items-center">
                        <div class="col-sm">
                            <div>
                                <h5 class="card-title mb-0">Degree Levels List</h5>
                        ...
                        <div class="col-sm-auto">
                            <div class="d-flex flex-wrap align-items-start gap-2">
                                ...
                                <button type="button" class="btn btn-primary add-btn"
                                    data-action="{{ route('settings.degree-levels.store') }}" data-bs-toggle="modal"
                        ...
                        <tbody class="list" id="table-list">
                            @foreach ($degree_levels as $degree_level)
                                <tr>
                                    ...
                                    <td>{{ $degree_level->name }}</td>
                                    <td>{{ $degree_level->program->name }}</td>
                                    <td><span
                                            class="badge {{ $degree_level->status == 'active' ? 'bg-success-subtle text-success' : 'bg-danger-subtle text-danger' }}">{{ ucfirst($degree_level->status) }}</span>
                                    </td>
                                    <td>
                                        <ul class="list-inline hstack gap-2 mb-0">
                                            <li class="list-inline-item edit" data-bs-toggle="tooltip"
                                                ...
                                                <a href="#showModal" data-bs-toggle="modal"
                                                    data-id="{{ $degree_level->id }}" data-name="{{ $degree_level->name }}"
                                                    data-status="{{ $degree_level->status }}"
                                                    data-description="{{ $degree_level->description }}"
                                                    data-program_id="{{ $degree_level->program_id }}"
                                                    data-action="{{ route('settings.degree-levels.update', ['degree_level' => $degree_level->id]) }}"
                                            ...
                                            <li class="list-inline-item" data-bs-toggle="tooltip" data-bs-trigger="hover"
                                                ...
                                                <a class="text-danger d-inline-block remove-item-btn" data-bs-toggle="modal"
                                                    data-id="{{ $degree_level->id }}" href="#deleteRecordModal">
                        ...
                        <div class="modal-content">
                            ...
                            <form class="tablelist-form" method="POST" autocomplete="off">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="mb-3">
                                        <label for="name" class="form-label">Degree Level Name</label>
    ...
    <script>
        $(document).ready(function() {
            $(document).on('click', '#create-btn', function() {
                document.getElementById("exampleModalLabel").innerHTML = "Add Degree Level";
            ...
            $(document).on('click', '.edit-btn', function() {
                ...
                document.getElementById("exampleModalLabel").innerHTML = "Edit Degree Level";;
            ...
            $(document).on('click', '.remove-item-btn', function() {
                var route = "{{ route('settings.degree-levels.destroy', ['degree_level' => ':id']) }}";
    ...
    @if ($errors->any())
        @if (old('id'))
            <script>
                ...
                var id = "{{ old('id') }}";
                var route = "{{ route('settings.degree-levels.update', ['degree_level' => ':id']) }}";
