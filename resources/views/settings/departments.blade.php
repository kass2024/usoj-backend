                        ...
                        <div class="col-sm-auto">
                            <div class="d-flex flex-wrap align-items-start gap-2">
                                ...
                                <button type="button" class="btn btn-primary add-btn"
                                    data-action="{{ route('settings.departments.store') }}" data-bs-toggle="modal"
                    ...
                    <table class="table align-middle" style="width: 100%">
                        <thead class="table-light text-muted">
                            <tr>
                                ...
                                <th class="sort" data-sort="name">Abbr</th>
                        ...
                        <tbody class="list" id="table-list">
                            @foreach ($departments as $department)
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="badge {{ $department->status == 'active' ? 'bg-success-subtle text-success' : 'bg-danger-subtle text-danger' }}">{{ ucfirst($department->status) }}</span>
                                    </td>
                                    <td>
                                        <ul class="list-inline hstack gap-2 mb-0">
                                            <li class="list-inline-item edit" data-bs-toggle="tooltip"
                                                data-bs-trigger="hover" data-bs-placement="top" title="Edit">
                                                <a href="#showModal" data-bs-toggle="modal" data-id="{{ $department->id }}"
                                            ...
                                            <li class="list-inline-item" data-bs-toggle="tooltip" data-bs-trigger="hover"
                                                ...
                                                <a class="text-danger d-inline-block remove-item-btn" data-bs-toggle="modal"
                                                    data-id="{{ $department->id }}" href="#deleteRecordModal">
                        ...
                        <div class="modal-content">
                            ...
                            <form class="modal-form" method="POST" autocomplete="off">
                                ...
                                <div class="modal-body">
                                    ...
                                    <input type="hidden" value="{{ $school }}" name="school_id" />
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="abbr" value="{{ old('abbr') }}" id="abbr"
                                            class="form-control" placeholder="Enter abbr" />
                                        @error('abbr')
                                            <span class="text-danger">{{ $message }}</span>
                                        @enderror
                                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.remove-item-btn', function() {
                var route = "{{ route('settings.departments.destroy', ['department' => ':id']) }}";
    ...
    @if ($errors->any())
        @if (old('id'))
            <script>
                ...
                var id = "{{ old('id') }}";
                var route = "{{ route('settings.departments.update', ['department' => ':id']) }}";
        ...
        @else
            <script>
                document.getElementById("exampleModalLabel").innerHTML = "Add Department";
                const route = "{{ route('settings.departments.store') }}";
