...
<style>
  ...
  .exam-col{display:flex}
  .card-exam{height:100%;display:flex;flex-direction:column;background:#fff;
    border:1px solid #e9ecef;border-radius:1rem;overflow:hidden;min-height:340px}
  .card-exam .card-header{background:#f8fafc;border-bottom:1px solid #e9ecef;padding:.75rem 1rem}
  .card-exam .card-body{padding:1rem}
  .card-exam .card-footer{background:#fff;border-top:0;padding:0 1rem 1rem}
  ...
  .btn-grad-primary{color:#fff;border:0;background:linear-gradient(135deg,#007a33 0%,#00a651 100%);box-shadow:0 6px 18px rgba(0,122,51,.25)}
  .btn-grad-primary:hover{filter:brightness(1.05);box-shadow:0 8px 22px rgba(0,122,51,.32)}
  .btn-grad-success{color:#fff;border:0;background:linear-gradient(135deg,#16a34a 0%,#22c55e 100%);box-shadow:0 6px 18px rgba(22,163,74,.25)}
  ...
  .stat-tile{display:flex;gap:.75rem;align-items:center;padding:.6rem .75rem;border-radius:.9rem;border:1px solid #edf2f7;background:linear-gradient(180deg,#ffffff 0%, #f8fafc 100%);box-shadow:0 2px 8px rgba(2,6,23,0.04)}
  .stat-ico{flex:0 0 40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#e9f0ff,#f5f8ff);color:#2563eb;border:1px solid #e6eeff}
...
@php
  ...
  /** Expect $exams from controller */
  $prepared = $exams->map(function($exam){
  ...
  <div class="filterbar mb-4">
    ...
    <div class="d-flex gap-2">
      ...
      <div class="align-self-end">
        <button type="button" id="resetFilters" class="btn btn-outline-primary">Reset</button>
      </div>
    </div>
  </div>

  @if($prepared->isEmpty())
    <div class="empty">No exams yet.</div>
  @else
    <div id="examGrid" class="row g-3">
      @foreach($prepared as $row)
        @php $e = $row['exam']; @endphp
        <div class="col-12 col-sm-6 exam-col exam-item"
             data-status="{{ $row['statusKey'] }}"
             data-year="{{ $row['year'] ?? '' }}"
             data-start="{{ $row['start_iso'] ?? '' }}"
             data-end="{{ $row['end_iso'] ?? '' }}"
             data-search="{{ e($row['searchBlob']) }}">
          <div class="card card-exam shadow-sm w-100">
            <div class="card-header">
              <div class="d-flex align-items-start justify-content-between">
                <div>
                  ...
                  <h6 class="mb-0" data-title>{{ $e->title }}</h6>
                  <div class="meta" data-meta>
                    {{ $e->module->course->name }} · Module: {{ $e->module->code ?? $e->module->name }}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="card-body d-flex flex-column gap-2">
              @if(!empty($e->description))
                <div class="text-truncate" data-desc title="{{ $e->description }}">{{ $e->description }}</div>
              @endif
                ...
                <div class="stat-tile">
                  ...
                  <div class="stat-main">
                    ...
                    @if($row['submission'])
                      ...
                      <span class="stat-sub">{{ $row['percent'] }}%</span>
                    @else
              ...
              <div class="meta mt-1">
                ...
                @if($row['start']) <span class="text-muted"> · {{ $row['start']->diffForHumans() }}</span>@endif
              </div>
            ...
            <div class="card-footer mt-auto">
              @if(!$row['submission'])
                <a href="{{ route('student.exams.submission', $e->id) }}" class="btn btn-grad-primary btn-pill w-100 btn-icon start-btn" title="Start exam">
                  <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M8 5v14l11-7z"/></svg>
                  <span>Start</span>
                </a>
              @else
                <a href="" class="btn btn-grad-success btn-pill w-100 btn-icon" title="Preview submission">
    ...
    <div id="noResults" class="empty mt-2" style="display:none;">No exams match your filters.</div>
  @endif
</div>
@endsection
...
(function(){
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    var cards = Array.prototype.slice.call(grid.querySelectorAll('.exam-item'));
    ...
    function getBlob(el){
      ...
      if(v && v !== 'null' && v !== 'undefined') return norm(v);
      var t = '';
      var title = el.querySelector('[data-title]'); if(title) t += ' ' + title.textContent;
      var meta  = el.querySelector('[data-meta]');  if(meta)  t += ' ' + meta.textContent;
      var desc  = el.querySelector('[data-desc]');  if(desc)  t += ' ' + desc.textContent;
      return norm(t);
    }
    ...
    var t;
    if(qSearch){
      qSearch.addEventListener('input', function(){ clearTimeout(t); t = setTimeout(applyFilters, 120); });
    }
    if(clearBtn){
      clearBtn.addEventListener('click', function(){
        if(qSearch){ qSearch.value=''; qSearch.focus(); }
        applyFilters();
      });
    }
    if(resetBtn){
    ...
    // Countdown (start→end: show time until open if upcoming, time until end if open, 0 if closed)
